...
val segmentReady: Boolean get() = sinetHandle != 0L
    ...
    private fun loadAll(context: Context) {
        ...
        loadModel(context, "models/sinet.tflite", NativeSegment.MODEL_SINET, delegateFlags) { sinetHandle = it }
        loadModel(context, "models/mediapipe_selfie.tflite", NativeSegment.MODEL_MEDIAPIPE_SELFIE, delegateFlags) { mediapipeHandle = it }
    }
    ...
    private fun loadModel(context: Context, path: String, type: Int, delegateFlags: Int, onSuccess: (Long) -> Unit) {
        try {
            ...
            val handle = NativeSegment.loadModel(buf, type, delegateFlags)
            if (handle != 0L) {
                onSuccess(handle)
    ...
    fun segmentFrameWithMediapipe(rgba: ByteBuffer, width: Int, height: Int, mask: ByteBuffer): Boolean {
        val h = if (mediapipeHandle != 0L) mediapipeHandle else sinetHandle
